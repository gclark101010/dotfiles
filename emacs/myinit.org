...
#+begin_src emacs-lisp
  (defun my/formatted-copy ()
    "Export region to HTML and copy as RTF to the system clipboard."
    (interactive)
      ...
      (let* ((buf (org-export-to-buffer 'html "*Formatted Copy*" nil nil t t))
        ...
        (with-current-buffer buf
          (shell-command-on-region
           (point-min) (point-max)
           "textutil -stdin -format html -convert rtf -stdout | pbcopy -Prefer rtf"))
        (kill-buffer buf))))

  (global-set-key (kbd "C-c M-c") 'my/formatted-copy)
#+end_src
...
Show buffer file path (mirrors Vim's =C-g=, which is reserved for =keyboard-quit= in Emacs)
#+begin_src emacs-lisp
  (evil-define-key 'normal 'global (kbd "g C-g")
...
This code is from [[https://stackoverflow.com/a/32914548][StackOverflow]] for opening Github PR in browser after pushing branch
#+begin_src emacs-lisp
  ...
  (user-error "Remote is not on GitHub: %s" repo))))
